      ...
      <div className="relative z-10 min-h-screen flex flex-col items-center justify-start px-4 py-10">
        ...
        <ol className="space-y-4 w-full max-w-2xl">
          {album.tracks.items.map((track, idx) => (
            <li
              key={track.id}
              className="bg-white/90 rounded-xl shadow flex items-center gap-4 p-4 hover:bg-green-50 transition"
            >
              ...
              {track.preview_url ? (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-white font-semibold ${
                    currentTrack?.url === track.preview_url && isPlaying
                      ? "bg-red-500"
                      : "bg-green-500"
                  } hover:bg-green-700 transition`}
                >
                  {currentTrack?.url === track.preview_url && isPlaying ? "Pause" : "Play"}
                </button>
              ) : (
